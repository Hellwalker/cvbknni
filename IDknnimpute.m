...
function[ImpData, paraSetting] = IDknnimpute(Data)
...
% median and mean matrix
% medM = median(orderedData, 'omitnan');
% meanM = mean(orderedData,'omitnan');
...
for itemp = 1:size(cmis,1)
    ...
    for jtemp = 1:size(tempMisColIndex,1)
        ...
        [ImpData(temp, px),paraSetting(m:(m+delta),:)] = feval(@IDknniKernel, RestData(:,2:end), xtest(:,2:end), Y);
        tempIn(temp,1) = 0;
        m = m+delta+1;
    end
end
