% feature relevance detecting by Fisher Score
% pairwise missing value deletion modified by Hellwalker
...
for i = 1:numF
    ...
    % pairwise deletion embedded
    
    f_i = X(:,i);
    f_i = f_i(~isnan(f_i),:);
    
    % numC equals to the class number
    y = Y(~isnan(f_i),:);
    numC = max(Y);
